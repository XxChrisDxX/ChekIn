...
public class Database {
	...
	public void deleteEventList(String event) throws IOException, NullPointerException{
		...
		add.append(line.substring(line.indexOf(event) + event.length(), line
				.length()));
		add.append("\n");
		out.write(add.toString());
		while ((line = input.readLine()) != null) {
	...
	public ArrayList<String> readEventStats() throws IOException, DateTimeParseException {
		...
		while ((line = input.readLine()) != null) {
			...
			age = Integer.toString(Period.between(dobDate, LocalDate.now())
					.getYears());
			while (index != limit) {
				event = line.substring(index + 1, line.indexOf(',', index + 1));
				eventdate = event.substring(event.indexOf(' ') + 1, event
						.length());
	...
	public ArrayList<String> readEvent(String data) throws IOException {
		...
		while ((line = input.readLine()) != null) {
			...
			if (name.equalsIgnoreCase(data)) {
				...
				limit = line.lastIndexOf(',');
				while (index != limit) {
					event = line.substring(index + 1, line.indexOf(',', index
							+ 1));
					events.add(event);
					index = line.indexOf(',', index + 1);
				}
	...
	public String readPhone(String data) throws IOException {
		FileReader file = new FileReader("database.dat");
		BufferedReader input = new BufferedReader(file);
		String name = "";
		String Phone = "";
		...
		while ((line = input.readLine()) != null) {
			...
			for (int i = 0; i < length; i++) {
				if (line.charAt(i) == ':') {
					name = line.substring(0, i);
					if (data.equalsIgnoreCase(name)) {
						tempIndex = line.indexOf(':', i + 1);
						phoneIndex = line.indexOf(':', tempIndex + 1);
						Phone = line.substring(tempIndex + 1, phoneIndex);
						file.close();
						input.close();
						return Phone;
					} else
	...
	public void writePhone(String data, String phone) throws IOException {
		StringBuilder add = new StringBuilder(150);
		File main = new File("database.dat");
		File temp = new File("temp.dat");
		BufferedReader input = new BufferedReader(new FileReader(main));
		...
		while ((line = input.readLine()) != null) {
			int length = line.indexOf(':');
			tempIndex = line.indexOf(':', length + 1);
			name = line.substring(0, length);
			if (name.equalsIgnoreCase(data)) {
				add.append(line.substring(0, tempIndex));
				add.append(":");
				add.append(phone);
		...
		while ((line = input.readLine()) != null) {
			...
			for (int i = 0; i < length; i++) {
				if (line.charAt(i) == ':') {
					name = line.substring(0, i);
					if (data.equalsIgnoreCase(name)) {
						birth = line.substring(line.lastIndexOf(':') + 1, line
								.indexOf('%'));
						file.close();
						input.close();
						return birth;
	...
	public void writePermissions(String data, int Permission)
			throws IOException {
		StringBuilder add = new StringBuilder(150);
		File main = new File("database.dat");
		File temp = new File("temp.dat");
